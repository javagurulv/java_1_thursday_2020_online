...
class BookDatabaseImpl implements BookDatabase {
    ...
    public Map<String, List<Book>> getAuthorToBooksMap() {
        Map<String, List<Book>> authorAllBooks = new HashMap<>();

        for (Book book : books) {
            List<Book> allBooks = books.stream().filter(b -> b.getAuthor().equals(book.getAuthor())).collect(Collectors.toList());
            authorAllBooks.put(book.getAuthor(), allBooks);
        }
    ...
    public Map<String, Integer> getEachAuthorBookCount() {
        ...
        Map<String, Integer> authorAllBooks = new HashMap<>();

        for (Book book : books) {
            int countBooks = (int) books.stream().filter(b -> b.getAuthor().equals(book.getAuthor())).count();

            authorAllBooks.put(book.getAuthor(), countBooks);
        }
        return authorAllBooks;
    }
}
