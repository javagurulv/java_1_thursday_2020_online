package student_aleksey_kodin.lesson12.level6.task_45;
...
class ProductValidatorImplTest {
    ...
    public void rule10_v1() {
        Product product = new Product("", 0, "Hi%");
        List<ValidationException> exceptions = validator.validate(product);
        checkResult(exceptions.size() == 3, "rule10_v1");

        Optional<ValidationException> rule1 = exceptions.stream().filter(e -> e.getRuleTitle().equals("RULE-1")).findAny();
        Optional<ValidationException> rule2 = exceptions.stream().filter(e -> e.getRuleTitle().equals("RULE-7")).findAny();
        Optional<ValidationException> rule3 = exceptions.stream().filter(e -> e.getRuleTitle().equals("RULE-9")).findAny();

        checkResult(rule1.get().getRuleTitle().equals("RULE-1"), "rule10_v1");
        checkResult(rule2.get().getRuleTitle().equals("RULE-7"), "rule10_v1");
        checkResult(rule3.get().getRuleTitle().equals("RULE-9"), "rule10_v1");
    }

    public void rule10_v2() {
        Product product = new Product("", 0, "Hi");
        List<ValidationException> exceptions = validator.validate(product);
        checkResult(exceptions.size() == 2, "rule10_v2");

        Optional<ValidationException> rule1 = exceptions.stream().filter(e -> e.getRuleTitle().equals("RULE-1")).findAny();
        Optional<ValidationException> rule2 = exceptions.stream().filter(e -> e.getRuleTitle().equals("RULE-7")).findAny();

        checkResult(rule1.get().getRuleTitle().equals("RULE-1"), "rule10_v2");
        checkResult(rule2.get().getRuleTitle().equals("RULE-7"), "rule10_v2");
    }
