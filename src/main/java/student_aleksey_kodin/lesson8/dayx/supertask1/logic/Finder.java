...
public  class Finder {
    ...
    public static Boolean isReaderNoReturnBook(Library library,Reader reader,LocalDate datePattern) {
    List<Book> takenBooks = getAllBooksForReader(library,reader);
        for (Book takenBook : takenBooks) {
            if (datePattern.isAfter(getFinishDateForBook(library, takenBook))) {
                library.noReturnBooks.put(reader, takenBook);
                return true;
            }
        }
    return false;
