...
class BookReaderImpl implements BookReader {
    ...
    public String[] getAllReadBooks() {
        ...
        for (Book book : books) {
            if (book.isMarkedAsRead()) {
                readBooks.add(book.toString());
            }
        }
        return readBooks.toArray(new String[] {});
    }
    ...
    public String[] getAllUnreadBooks() {
        ...
        for (Book book : books) {
            if (!book.isMarkedAsRead()) {
                unreadBooks.add(book.toString());
            }
        }
        return unreadBooks.toArray(new String[] {});
    }
    ...
    private int getAllReadBooksCount() {
        int count = 0;
        for (Book book : books) {
            if (book.isMarkedAsRead()) {
                count++;
    ...
    private char[] getFirstFourLetters(String authorOrTitle) {
