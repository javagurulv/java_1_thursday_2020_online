...
class BookReaderImpl implements BookReader {
    ...
    @Override
    public String[] getReadBooksListFromLibrary () {
        List<String> readBooksListFromLibrary = new ArrayList<>();
        for (Book eachbook : readerLibrary) {
            if (eachbook.isRead()) {
                readBooksListFromLibrary.add(eachbook.getBookTitle()
                        + " [" + eachbook.getBookAuthor()
                        + "]");
            }
        } return readBooksListFromLibrary.toArray(new String []{});
    }


    @Override
    public String[] getNotReadBooksListFromLibrary () {
        List<String> notReadBooksListFromLibrary = new ArrayList<>();
        for (Book eachbook : readerLibrary) {
            if (!eachbook.isRead()) {
                notReadBooksListFromLibrary.add(eachbook.getBookTitle()
                        + " [" + eachbook.getBookAuthor()
                        + "]");
            }
        } return notReadBooksListFromLibrary.toArray(new String []{});
    }
