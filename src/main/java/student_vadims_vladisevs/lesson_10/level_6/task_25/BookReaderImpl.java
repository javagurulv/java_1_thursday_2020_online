...
class BookReaderImpl implements BookReader {
    ...
    @Override
    public boolean completedRead(Book book, Library library) {
        if (isBookInLibrary(book, library)){
            book.setRead(true);
            return true;
        }
    ...
    @Override
    public boolean notCompletedRead(Book book, Library library) {
        if (isBookInLibrary(book, library)){
            book.setRead(false);
            return true;
        }
    ...
    @Override
    public String[] listOfCompletedBooks(Library library) {
        ArrayList<String> listOfFinishedBooks = isFinishedBooks(library);
        return copyListToArray(listOfFinishedBooks);
    }

    private ArrayList<String> isFinishedBooks(Library library){
        ArrayList<String> tempList = new ArrayList<>();
        for (Book tempBook : library.getBooks()){
            if (tempBook.isRead()){
                tempList.add(tempBook.getTitle() + " [" + tempBook.getAuthor() + "]");
            }
        }
        return tempList;
    }
    ...
    @Override
    public String[] listOfNotCompletedBooks(Library library) {
        ArrayList<String> listOfNotFinishedBooks = notFinishedBooks(library);
        return copyListToArray(listOfNotFinishedBooks);
    }



    private ArrayList<String> notFinishedBooks(Library library){
        ArrayList<String> tempList = new ArrayList<>();
        for (Book tempBook : library.getBooks()){
            if (!tempBook.isRead()){
                tempList.add(tempBook.getTitle() + " [" + tempBook.getAuthor() + "]");
    ...
    private String[] copyListToArray(ArrayList<String> list){
